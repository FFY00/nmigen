...
class _VCDWaveformWriter(_WaveformWriter):
    ...
    def __init__(self, fragment, *, vcd_file, gtkw_file=None, traces=()):
        ...
        self.vcd_writer = vcd_file and VCDWriter(self.vcd_file, timescale="100 ps", comment="Generated by nMigen")
                ...
                while True:
                    try:
                        ...
                        vcd_var = self.vcd_writer.register_var(
                            scope=var_scope, name=var_name_suffix, var_type=var_type, size=var_size, init=var_init
                        )
                        break
                    except KeyError:
...
class _SimulatorState:
    ...
    def add_trigger(self, process, signal, *, trigger=None):
        ...
        assert (process not in self.slots[index].waiters or self.slots[index].waiters[process] == trigger)
...
class Simulator:
    ...
    def _check_process(self, process):
        if not (inspect.isgeneratorfunction(process) or inspect.iscoroutinefunction(process)):
            raise TypeError("Cannot add a process {!r} because it is not a generator function".format(process))
        return process

    def _add_coroutine_process(self, process, *, default_cmd):
        self._processes.add(PyCoroProcess(self._state, self._fragment.domains, process, default_cmd=default_cmd))
    ...
    def add_clock(self, period, *, phase=None, domain="sync", if_exists=False):
        ...
        if domain in self._clocked:
            raise ValueError("Domain {!r} already has a clock driving it".format(domain.name))
        ...
        while not converged:
            ...
            for waveform_writer in self._waveform_writers:
                for signal_state in self._state.pending:
                    waveform_writer.update(self._state.timeline.now, signal_state.signal, signal_state.curr)
    ...
    def write_vcd(self, vcd_file, gtkw_file=None, *, traces=()):
        ...
        waveform_writer = _VCDWaveformWriter(self._fragment, vcd_file=vcd_file, gtkw_file=gtkw_file, traces=traces)
