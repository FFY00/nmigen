...
class PinsTestCase(FHDLTestCase):
    ...
    def test_wrong_names(self):
        with self.assertRaises(TypeError, msg="Names must be a whitespace-separated string, not ['A0', 'A1', 'A2']"):
            p = Pins(["A0", "A1", "A2"])

    def test_wrong_dir(self):
        with self.assertRaises(TypeError, msg="Direction must be one of \"i\", \"o\", \"oe\", or \"io\", not 'wrong'"):
    ...
    def test_wrong_map_names(self):
        ...
        with self.assertRaises(
            NameError, msg="Resource (pins io pmod_0:0 pmod_0:1 pmod_0:2) refers to nonexistent "
            "connector pin pmod_0:1"
        ):
            p.map_names(mapping, p)

    def test_wrong_assert_width(self):
        with self.assertRaises(AssertionError, msg="3 names are specified (0 1 2), but 4 names are expected"):
...
class AttrsTestCase(FHDLTestCase):
    ...
    def test_wrong_value(self):
        with self.assertRaises(TypeError, msg="Value of attribute FOO must be None, int, str, or callable, not 1.0"):
            a = Attrs(FOO=1.0)
...
class SubsignalTestCase(FHDLTestCase):
    def test_basic_pins(self):
        ...
        self.assertEqual(repr(s), "(subsignal a (pins io A0) (attrs IOSTANDARD='LVCMOS33'))")
    ...
    def test_basic_subsignals(self):
        s = Subsignal("a", Subsignal("b", Pins("A0")), Subsignal("c", Pins("A1")))
        self.assertEqual(repr(s), "(subsignal a (subsignal b (pins io A0)) " "(subsignal c (pins io A1)))")

    def test_attrs(self):
        s = Subsignal(
            "a", Subsignal("b", Pins("A0")), Subsignal("c", Pins("A0"), Attrs(SLEW="FAST")), Attrs(IOSTANDARD="LVCMOS33")
        )
    ...
    def test_wrong_io(self):
        with self.assertRaises(
            TypeError, msg="Constraint must be one of Pins, DiffPairs, Subsignal, Attrs, or Clock, "
            "not 'wrong'"
        ):
            s = Subsignal("a", "wrong")
    ...
    def test_wrong_clock(self):
        with self.assertRaises(
            TypeError, msg="Clock constraint can only be applied to Pins or DiffPairs, not "
            "(subsignal b (pins io A0))"
        ):
            s = Subsignal("a", Subsignal("b", Pins("A0")), Clock(1e6))

    def test_wrong_clock_many(self):
        with self.assertRaises(ValueError, msg="Clock constraint can be applied only once"):
...
class ResourceTestCase(FHDLTestCase):
    def test_basic(self):
        r = Resource(
            "serial", 0, Subsignal("tx", Pins("A0", dir="o")), Subsignal("rx", Pins("A1", dir="i")),
            Attrs(IOSTANDARD="LVCMOS33")
        )
        self.assertEqual(
            repr(r), "(resource serial 0"
...
class ConnectorTestCase(FHDLTestCase):
    def test_string(self):
        ...
        self.assertEqual(repr(c), "(connector pmod 0 1=>A0 2=>A1 3=>A2 4=>A3 7=>A4 8=>A5 9=>A6 10=>A7)")
    ...
    def test_wrong_io(self):
        with self.assertRaises(TypeError, msg="Connector I/Os must be a dictionary or a string, not []"):
            Connector("pmod", 0, [])

    def test_wrong_dict_key_value(self):
        with self.assertRaises(TypeError, msg="Connector pin name must be a string, not 0"):
            Connector("pmod", 0, {0: "A"})
        with self.assertRaises(TypeError, msg="Platform pin name must be a string, not 0"):
