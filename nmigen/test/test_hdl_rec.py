...
class LayoutTestCase(FHDLTestCase):
    ...
    def test_fields(self):
        layout = Layout.cast([("cyc", 1), ("data", signed(32)), ("stb", 1, DIR_FANOUT), ("ack", 1, DIR_FANIN),
                              ("info", [
                                  ("a", 1),
                                  ("b", 1),
                              ])])
    ...
    def test_slice_tuple(self):
        layout = Layout.cast([("a", 1), ("b", 2), ("c", 3)])
        expect = Layout.cast([("a", 1), ("c", 3)])
        self.assertEqual(layout["a", "c"], expect)

    def test_repr(self):
        self.assertEqual(
            repr(Layout([("a", unsigned(1)), ("b", signed(2))])), "Layout([('a', unsigned(1)), ('b', signed(2))])"
        )
        self.assertEqual(
            repr(Layout([("a", unsigned(1)), ("b", [("c", signed(3))])])), "Layout([('a', unsigned(1)), "
            "('b', Layout([('c', signed(3))]))])"
        )

    def test_wrong_field(self):
        with self.assertRaises(
            TypeError, msg="Field (1,) has invalid layout: should be either (name, shape) or "
            "(name, shape, direction)"
        ):
            Layout.cast([(1, )])

    def test_wrong_name(self):
        with self.assertRaises(TypeError, msg="Field (1, 1) has invalid name: should be a string"):
            Layout.cast([(1, 1)])

    def test_wrong_name_duplicate(self):
        with self.assertRaises(NameError, msg="Field ('a', 2) has a name that is already present in the layout"):
            Layout.cast([("a", 1), ("a", 2)])

    def test_wrong_direction(self):
        with self.assertRaises(
            TypeError, msg="Field ('a', 1, 0) has invalid direction: should be a Direction "
...
class RecordTestCase(FHDLTestCase):
    ...
    def test_wrong_field(self):
        ...
        with self.assertRaises(AttributeError, msg="Record 'r' does not have a field 'en'. Did you mean one of: stb, ack?"):
            r["en"]
        with self.assertRaises(AttributeError, msg="Record 'r' does not have a field 'en'. Did you mean one of: stb, ack?"):
            r.en
    ...
    def test_wrong_field_unnamed(self):
        ...
        with self.assertRaises(
            AttributeError, msg="Unnamed record does not have a field 'en'. Did you mean one of: stb, ack?"
        ):
            r.en
    ...
    def test_construct_with_fields(self):
        ns = Signal(1)
        nr = Record([("burst", 1)])
        r = Record([("stb", 1), ("info", [("burst", 1)])], fields={"stb": ns, "info": nr})
...
class ConnectTestCase(FHDLTestCase):
    ...
    def test_wrong_include_exclude(self):
        ...
        with self.assertRaises(AttributeError, msg="Cannot include field 'foo' because it is not present in record 'core'"):
            core.connect(periph, include={"foo": True})

        with self.assertRaises(AttributeError, msg="Cannot exclude field 'foo' because it is not present in record 'core'"):
    ...
    def test_wrong_direction(self):
        ...
        with self.assertRaises(
            TypeError, msg="Cannot connect field 'x' of unnamed record because it does not have "
            "a direction"
        ):
