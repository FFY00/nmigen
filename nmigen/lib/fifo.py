...
class FIFOInterface:
    ...
    def __init__(self, *, width, depth, fwft):
        if not isinstance(width, int) or width < 0:
            raise TypeError("FIFO width must be a non-negative integer, not {!r}".format(width))
        if not isinstance(depth, int) or depth < 0:
            raise TypeError("FIFO depth must be a non-negative integer, not {!r}".format(depth))
        self.width = width
        self.depth = depth
        self.fwft = fwft
...
class SyncFIFO(Elaboratable, FIFOInterface):
    ...
    def elaborate(self, platform):
        ...
        m.d.comb += [self.w_rdy.eq(self.level != self.depth), self.r_rdy.eq(self.level != 0)]
        ...
        r_port = m.submodules.r_port = storage.read_port(domain="comb" if self.fwft else "sync", transparent=self.fwft)
        produce = Signal(range(self.depth))
        consume = Signal(range(self.depth))

        m.d.comb += [w_port.addr.eq(produce), w_port.data.eq(self.w_data), w_port.en.eq(self.w_en & self.w_rdy)]
        with m.If(do_write):
...
class SyncFIFOBuffered(Elaboratable, FIFOInterface):
    ...
    def elaborate(self, platform):
        ...
        m.submodules.unbuffered = fifo = SyncFIFO(width=self.width, depth=self.depth - 1, fwft=False)
...
class AsyncFIFO(Elaboratable, FIFOInterface):
    ...
    def __init__(self, *, width, depth, r_domain="read", w_domain="write", exact_depth=False):
        if depth != 0:
            try:
                ...
                raise ValueError(
                    "AsyncFIFO only supports depths that are powers of 2; requested "
                    "exact depth {} is not".format(depth)
                ) from None
        else:
            depth_bits = 0
        super().__init__(width=width, depth=depth, fwft=True)
    ...
    def elaborate(self, platform):
        ...
        r_empty = Signal()
        m.d.comb += [
            w_full.eq((produce_w_gry[-1] != consume_w_gry[-1]) & (produce_w_gry[-2] != consume_w_gry[-2])
        ...
        r_port = m.submodules.r_port = storage.read_port(domain=self._r_domain, transparent=False)
        m.d.comb += [
...
class AsyncFIFOBuffered(Elaboratable, FIFOInterface):
    ...
    def __init__(self, *, width, depth, r_domain="read", w_domain="write", exact_depth=False):
        if depth != 0:
            try:
                ...
                raise ValueError(
                    "AsyncFIFOBuffered only supports depths that are one higher "
                    "than powers of 2; requested exact depth {} is not".format(depth)
                ) from None
        super().__init__(width=width, depth=depth, fwft=True)
    ...
    def elaborate(self, platform):
        ...
        m.submodules.unbuffered = fifo = AsyncFIFO(
            width=self.width, depth=self.depth - 1, r_domain=self._r_domain, w_domain=self._w_domain
        )
