...
class MustUse:
    ...
    def __new__(cls, *args, src_loc_at=0, **kwargs):
        ...
        self._MustUse__context = dict(filename=frame.f_code.co_filename, lineno=frame.f_lineno, source=self)
        return self
    ...
    def __del__(self):
        ...
        if hasattr(self, "_MustUse__used") and not self._MustUse__used:
            if get_linter_option(self._MustUse__context["filename"], self._MustUse__warning.__name__, bool, True):
                warnings.warn_explicit(
                    "{!r} created but never used".format(self), self._MustUse__warning, **self._MustUse__context
                )
